...
export class Form<ValueType extends object = any> {
  ...
  indexes: Map<string, string> = new Map() // filed key集合
  disposers: (() => void)[] = []
  ...
  protected initialize(props: IFormProps) {
    ...
    this.display = this.props.display || 'visible' // 最外层还控制显示和隐藏？
    this.pattern = this.props.pattern || 'editable' // 编辑状态
    this.editable = this.props.editable // 编辑状态
    this.disabled = this.props.disabled // 全局的disabled
    this.readOnly = this.props.readOnly // 全局的readOnly
